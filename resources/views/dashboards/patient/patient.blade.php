            ...
            <div class="topbar d-flex justify-content-end align-items-center p-3">
                <div class="d-flex align-items-center">
                    ...
                    <div class="profile-pic text-center">
                        <form id="profileImageForm" method="POST" action="{{ route('patient.upload.profile.image') }}" enctype="multipart/form-data">
                            @csrf
                            <label for="profileImageInput" style="cursor:pointer;">
                                <img src="{{ optional(\App\Models\Image::where('user_id', Auth::id())->where('role', 'patient')->first())->image_path ? asset('storage/' . \App\Models\Image::where('user_id', Auth::id())->where('role', 'patient')->first()->image_path) : '/images/patient.jpg' }}" alt="Profile" class="rounded-circle" width="120" height="120">
                            </label>
                ...
                @elseif ($section === 'settings')
                    <div class="settings-full">
                        ...
                        <div class="text-center mb-4">
                            <img src="{{ optional(\App\Models\Image::where('user_id', Auth::id())->where('role', 'patient')->first())->image_path ? asset('storage/' . \App\Models\Image::where('user_id', Auth::id())->where('role', 'patient')->first()->image_path) : '/images/patient.jpg' }}" alt="Profile" class="settings-profile-pic mb-2" width="120" height="120">
                            <div class="mt-2">
                                <form id="profileImageFormSettings" method="POST" action="{{ route('patient.upload.profile.image') }}" enctype="multipart/form-data" style="display:inline;">
                                    @csrf
                                    <input type="file" id="profileImageInputSettings" name="profile_image" accept="image/*" style="display:none;">
                                    <script>
                                        document.addEventListener('DOMContentLoaded', function() {
                                            var input = document.getElementById('profileImageInputSettings');
                                            if (input) {
                                                input.addEventListener('change', function() {
                                                    document.getElementById('profileImageFormSettings').submit();
                                                });
                                            }
                                        });
                                    </script>
                                    <button type="button" class="btn btn-success btn-sm" onclick="document.getElementById('profileImageInputSettings').click();">Upload</button>
                                </form>
                                <button type="button" class="btn btn-danger btn-sm">Remove</button>
                            </div>
                        </div>
                        <form class="settings-form mt-4" method="POST" action="{{ route('patient.update.settings') }}">
                            @csrf
                            <div class="row mb-3">
                                <div class="col-md-12">
                                    <label class="form-label">Name</label>
                                    <input type="text" class="form-control" name="name" value="{{ $patient->name ?? Auth::user()->name }}" required>
                                </div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-md-12">
